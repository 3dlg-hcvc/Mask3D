...
class SemanticSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if "train" in self.mode:
            ...
            #     if random() < 0.8:
            #         new_points = np.load(
            #             self.other_database[np.random.randint(0, len(self.other_database) - 1)]["filepath"]
            #         )
            #         (
            #             unlabeled_coords,
            #             unlabeled_color,
...
# def random_around_points(coordinates, color, normals, labels, rate=0.2, noise_rate=0, ignore_label=255):

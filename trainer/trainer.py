...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        for i in range(self.model.num_levels * self.model.num_decoders):
            if i not in self.config.general.ignore_mask_idx:
                ...
                aux_weight_dict.update({k + f"_{i}": 0.0 for k, v in weight_dict.items()})
    ...
    def training_step(self, batch, batch_idx):
        ...
        # try:
        output = self.forward(
            data, point2segment=[target[i]["point2segment"] for i in range(len(target))],
    ...
    #             )
    #
    #             gt_pcd_color.append(self.validation_dataset.map2color([label]).repeat(gt_pcd_pos[-1].shape[0], 1))
    ...
    #
    #             pred_sem_color.append(self.validation_dataset.map2color([label]).repeat(mask_coords.shape[0], 1))
    #
    #             pred_inst_color.append(
    #                 instances_colors[i % len(instances_colors)].unsqueeze(0).repeat(mask_coords.shape[0], 1)
    #             )
    #
    ...
    def eval_instance_step(
        ...
        prediction.append({"pred_logits": output["pred_logits"], "pred_masks": output["pred_masks"]})
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            #             query_pos=all_query_pos[bid][self.test_dataset.data[idx[bid]]["cond_inner"]]
